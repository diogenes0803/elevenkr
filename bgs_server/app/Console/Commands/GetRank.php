...
class GetRank extends Command
{
    ...
    {
        $apiUrl = env('ELEVEN_API', '')."/accounts/search/[BGS]";
        ...
        while(!$lastPage)
        {
            foreach($response['data'] as $user)
            {
                ...
                array_push($bgsUsers, $refinedUser);
            }
        ...
        $bgsUsers = collect($bgsUsers)->sortBy('world_rank')->toArray();
        $orderedUsers = [];
        $rankCount = 1;
        foreach($bgsUsers as $thisUser)
        {
